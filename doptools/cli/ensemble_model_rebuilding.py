        ...
        if os.path.isfile(trials_file):
            ...
            if "method" in df.columns:
                ...
                if method_value in trials_dict:
                    logging.critical(
                        f"Duplicate method detected: {method_value} in {trials_file}"
                    )
...
def select_best_CV_models(
    ...
    if highest_score < threshold:
        if not args.desperate:
            logging.info(
                f"The highest score across all the models is less than {threshold}. The script execution was stopped. However if you still want to rebuild those models rerun the script with the argument --desperate."
            )
            sys.exit(1)

    score_threshold = max(highest_score - 0.1, threshold)
    best_models_list = [
        max(models, key=lambda x: x["score"])
        for desc, models in models_by_desc.items()
        if any(m["score"] >= score_threshold for m in models)
    ]
    best_models = (
        pd.DataFrame(best_models_list)
...
def rebuild_and_evaluate_reg_model(
    ...
    if model_row["method"] == "RFR":
        model = RandomForestRegressor(
            ...
            max_features=(
                model_row["max_features"]
                if not pd.isna(model_row["max_features"])
                else None
            ),
        ...
        model = SVR(
            C=model_row["C"], kernel=model_row["kernel"], coef0=model_row["coef0"]
        )
...
def rebuild_model(
    ...
    """
    _, model_row = (
        model_row_tuple  # iterrows (from the main block) generates indexes, I don't need them
    )
    input_df = pd.DataFrame(list(shared_data))
...
def aggregate_CV_predictions(
    ...
    for _, model_row in best_models.iterrows():
        ...
        if method in trials_info_dict:
            ...
            if os.path.isfile(file_path):
                ...
                predicted_columns = [
                    col for col in trial_predictions.columns if ".predicted." in col
                ]

                # Aggregate across rows (compounds)
                if model_type == "reg":
                    # Calculate the mean for each row across predicted columns
                    aggregated_trial_prediction = (
                        trial_predictions[predicted_columns]
                        .mean(axis=1)
                        .rename(f"{trial_str}")
                    )
                else:
                    ...
                    mode_result = (
                        trial_predictions[predicted_columns].mode(axis=1)[0].astype(int)
                    )
    ...
    if model_type == "reg":
        ...
        final_df = pd.DataFrame(
            {property_col: actual_values, "AvgPred": final_mean, "StdDev": final_std}
        )
    else:
        # Calculate the mode and mode count across all trials
        mode_values = all_aggregated_predictions.mode(axis=1).iloc[:, 0].astype(int)
        class_probs = all_aggregated_predictions.apply(
            lambda row: (row.value_counts() / len(row)).round(2).to_dict(), axis=1
        )
        final_df = pd.DataFrame(
            {property_col: actual_values, "Mode": mode_values, "ClassProb": class_probs}
        )
...
def evaluate_AD_apply_model(
    ...
    def frag_ctrl(
        ...
        def conf_update(row: pd.Series) -> pd.Series:
            ...
            if isinstance(model_pipeline[0], ChythonLinear):
                ...
                test_fragmentor = ChythonLinear(
                    lower=model_pipeline[0].lower, upper=model_pipeline[0].upper
                )
            elif isinstance(model_pipeline[0], ChythonCircus):
                # Create a ChythonCircus test fragmentor
                test_fragmentor = ChythonCircus(
                    lower=model_pipeline[0].lower, upper=model_pipeline[0].upper
                )
            else:
                logging.error(
                    "Unsupported fragmentor type while using fragment control."
                )
                raise ValueError(
                    "Unsupported fragmentor type while using fragment control"
                )
            test_features = set(
                test_fragmentor.fit([row["Molecule"]]).get_feature_names()
            )
    ...
    def bbox(
        ...
        if pad_size > 0:
            ...
            max_train_descs = np.pad(
                max_train_descs, (0, pad_size), "constant", constant_values=(0, 0)
            )
    ...
    desc_space = os.path.basename(desc_file).split(".")[
        1
    ]  # Extract the descriptor space name

    model_files = glob.glob(
        f"{model_folder}/Model_*_*_{desc_space}_*.pkl"
    )  # Get all the files (it should be only 1) matching the criterium in a list
    if len(model_files) == 1:
        model_path = model_files[0]
    elif len(model_files) == 0:
        logging.error(
            f"No model found for the descriptor space '{desc_space}'. Please check the pkl_folder and descriptor space."
        )
        raise ValueError(
            f"No model found for the descriptor space '{desc_space}'. Please check the pkl_folder and descriptor space."
        )
        ...
        logging.error(
            f"Multiple models found for the descriptor space '{desc_space}'. Please ensure there is only one model per descriptor space."
        )
        raise ValueError(
            f"Multiple models found for the descriptor space '{desc_space}'. Please ensure there is only one model per descriptor space."
        )
    model_name = os.path.splitext(os.path.basename(model_path))[
        0
    ...
    if not (
        isinstance(model_pipeline[0], ChythonLinear)
        or isinstance(model_pipeline[0], ChythonCircus)
    ):
        ...
        pred_descs = np.array(
            p_descs_pipeline
        )  # convert the descriptors into np.array for bounding box assessment
        ...
        shared_predict_df = bbox(
            shared_predict_df, max_val_descs, pred_descs, desc_space
        )
        del train_descs, desc_svm, pred_descs, max_val_descs
        # Apply the model
        shared_predict_df[model_name] = Pipeline(model_pipeline.steps[1:]).predict(
            p_descs_pipeline
        )
    else:
        # Check the AD
        ...
        # Apply model
        shared_predict_df[model_name] = model_pipeline.predict(
            shared_predict_df["Molecule"]
        )
...
def aggregate_test_predictions(
    ...
    def in_AD_aggregation(df_row: pd.Series) -> pd.Series:
        ...
        valid_models = (
            []
        )  # models for which a given molecule is within the applicability domain
        for conf_col in conf_columns:  # conf_columns is declared in the parent function
            descriptor_space = conf_col.split("Conf-")[1]
            for (
                model_col
            ) in model_cols:  # model_cols is declared in the parent function
        ...
        if model_type == "reg":
            ...
            class_probs = (
                (df_row[valid_models].value_counts() / len(df_row[valid_models]))
                .round(2)
                .to_dict()
            )
    ...
    new_df = pd.DataFrame(
        {smiles: all_predictions[smiles] for smiles in ext_test_set_DF["SMILES"]}
    ).T
    ...
    DF_in_AD = ext_test_set_DF[
        ext_test_set_DF["Confidence"] != zero_confidence_value
    ].copy()
    DF_out_AD = ext_test_set_DF[
        ext_test_set_DF["Confidence"] == zero_confidence_value
    ].copy()
    ...
    if model_type == "reg":
        if (
            not DF_in_AD.empty
        ):  # calculate the mean and std. dev. for the compounds that are inside the AD
        ...
        if not DF_out_AD.empty:  # outside the AD calculations
            DF_out_AD["Mode"] = (
                DF_out_AD[model_cols].mode(axis=1).iloc[:, 0].astype(int)
            )
            DF_out_AD["ClassProbs"] = DF_out_AD[model_cols].apply(
                lambda row: (row.value_counts() / len(row)).round(2).to_dict(), axis=1
            )
            DF_out_AD.drop(columns=all_columns_to_drop, inplace=True)
...
def calculate_scores(
    ...
    if model_type == "class":
        ...
        scores["Balanced_Accuracy"] = np.round(
            balanced_accuracy_score(y_true, y_pred), 2
        )
...
def generate_confusion_matrix(
    ...
    if len(class_labels) > nb_classes:
        class_labels = class_labels[
            :nb_classes
        ]  # Use only the first nb_classes unique labels
    ...
    with open(os.path.join(outdir, "stats.txt"), "w") as f:
        ...
        f.write(
            f"Matthews Correlation Coefficient: {scores['Matthews_CorrCoef']:.2f}\n"
        )
...
if __name__ == "__main__":
    ...
    custom_descriptors = (
        args.custom_descriptor_col if args.custom_descriptor_col is not None else None
    )
    ...
    if model_type == "class":
        if args.class_info is None:
            logging.error(
                "--class_info must be specified when --model_type is 'class'."
            )
    ...
    # Load the input file
    input_df = (
        pd.read_csv(input_file).dropna(subset=[property_col]).reset_index(drop=True)
    )
    test_set_df = (
        pd.read_csv(ext_test_file).dropna(subset=[property_col]).reset_index(drop=True)
        if ext_test_file
        else None
    )
    if custom_descriptors is not None:
        # Check for the presence of custom_descriptors in input_df
        missing_columns_input = [
            col for col in custom_descriptors if col not in input_df.columns
        ]
        if missing_columns_input:
            logging.error(
                f"Missing columns in the input dataset: {missing_columns_input}"
            )
            sys.exit(1)
        # Check for the presence of custom_descriptors in test_set_df if it exists and is not empty
        if test_set_df is not None and not test_set_df.empty:
            missing_columns_test = [
                col for col in custom_descriptors if col not in test_set_df.columns
            ]
            if missing_columns_test:
                logging.error(
                    f"Missing columns in the external test set: {missing_columns_test}"
                )
                sys.exit(1)
    ...
    if "CSN" in uppercase_columns and "ID" in uppercase_columns:
        logging.error(
            "CSV file contains both 'CSN' and 'ID' columns. Please use only one as the molecule ID."
        )
        raise ValueError(
            "CSV file contains both 'CSN' and 'ID' columns. Please use only one as the molecule ID."
        )
        ...
        if "CSN" in uppercase_columns:
            mol_id = input_df.columns[uppercase_columns == "CSN"][
                0
            ]  # Get the actual column name that matches 'CSN'
        elif "ID" in uppercase_columns:
            mol_id = input_df.columns[uppercase_columns == "ID"][
                0
            ]  # Get the actual column name that matches 'ID'
    columns_to_keep = [structure_col, mol_id, property_col]
    if custom_descriptors is not None:
        columns_to_keep += custom_descriptors

    input_df = input_df.drop(
        columns=[col for col in input_df.columns if col not in columns_to_keep]
    )

    create_output_dir(model_folder)
    models_from_CV = select_best_CV_models(trials_info_dict, model_type, nb_classes)
    create_model_folder(
        desc_folder, model_folder, models_from_CV, input_df, test_set_df
    )
    input_df["Molecule"] = input_df[structure_col].apply(
        smiles
    )  # smiles comes from chython, transforms SMILES into MolObject
    if test_set_df is not None and not test_set_df.empty:
        test_set_df = test_set_df.drop(
            columns=[col for col in test_set_df.columns if col not in columns_to_keep]
        )
            ...
            if model_type == "reg":
                ...
                if not args.desperate:
                    if len(indices_to_drop) == len(models_from_CV):
                        logging.error(
                            "During models' evaluation with the external test set all the available individual models have shown a very poor score. If you still want to continue, re-run the script with the argument --desperate"
                        )
                        sys.exit(1)
                    models_from_CV.drop(index=indices_to_drop, inplace=True)
                if (
                    test_set_df is not None and not test_set_df.empty
                ):  # If I have test set, then I remove the CV score.
                    models_from_CV.drop(columns=["score"], inplace=True)
                    models_from_CV.rename(
                        columns={"evaluation_score": "score"}, inplace=True
                    )
                    models_from_CV.sort_values(
                        by="score", ascending=False, inplace=True
                    )
                best_models = models_from_CV.iloc[:10].reset_index(drop=True)
                rebuild_kwargs = {
                    k: v for k, v in kwargs.items() if k != "predict_df"
                }  # don't need to provide predict_df for the next function.
                ...
                svm_files = [
                    file for file in files_in_directory if file.endswith(".svm")
                ]
    ...
    if test_set_df is not None and not test_set_df.empty:
        with Manager() as manager:
            test_set_file_data = test_set_df.apply(
            ...
            all_results = dict(
                zip(
                    test_set_df["SMILES"],
                    [{} for _ in range(len(test_set_df["SMILES"]))],
                )
            )
            with mp.Pool(processes=args.parallel if args.parallel > 0 else 1) as pool:
                for desc_file in glob.glob(model_folder + "/*.svm"):
                    result = pool.apply_async(
                        evaluate_AD_apply_model, args=(desc_file, shared_molecules)
                    )
        ...
        final_DF = pd.concat(
            [final_DF, input_df[mol_id], input_df[structure_col]], axis=1
        )
    ...
    if len(final_DF) >= minimal_row_requirement:
        ...
        logging.info(
            "Not enough compounds are within the model's AD to compute sensible scores."
        )
    ...
    if final_DF_out_AD is not None and not final_DF_out_AD.empty:
        logging.info(
            "The predictions for compounds that are OUTSIDE of AD for the whole consensus model are saved to 'outside_of_AD_predictions.csv'"
        )
        out_AD_output_filename = os.path.join(
            model_folder, "outside_of_AD_predictions.csv"
        )
