...
def prepare_classification_plot(
    ...
    pos_label = [
        c
        for c in cv_res.columns
        if c.startswith(prop_name + ".predicted_prob.class_" + str(pos_class))
    ]
    if not pos_label:
        raise ValueError(
            "No property label corresponding to the given --pos_class argument"
        )
    ...
    for col in pos_probas.columns:
        ...
        if set(true_val) == set([0, 1]) or set(true_val) == set([-1, 1]):
            fpr, tpr, _ = roc_curve(
                true_val, pos_probas[col].values
            )  # , pos_label=pos_class)
        else:
            fpr, tpr, _ = roc_curve(
                true_val, pos_probas[col].values, pos_label=pos_class
            )
        roc_repeats[repeat] = {"fpr": fpr, "tpr": tpr, "auc": auc(fpr, tpr)}

    mean_fpr = np.linspace(0, 1, 100)
    interp_tpr = [
        np.interp(mean_fpr, roc_repeats[x]["fpr"], roc_repeats[x]["tpr"])
        for x in roc_repeats
    ]
...
def make_classification_plot(
    ...
    for rpt in roc_repeats:
        ax.plot(
            roc_repeats[rpt]["fpr"],
            roc_repeats[rpt]["tpr"],
            label=r"ROC repeat %.0f (AUC = %0.2f)"
            % (int(rpt), roc_repeats[rpt]["auc"]),
            alpha=0.3,
            lw=1,
        )
...
def plotter() -> None:
    ...
    parser.add_argument(
        "--pos_class", type=str, help="The label for the positive class"
    )
    ...
    if args.task == "R":
        fig, ax = make_regression_plot(
            datadir + "/predictions", errorbar=errorbar, stats=stats, title=title
        )
    elif args.task == "C":
        if not args.pos_class:
            raise ValueError(
                "Positive class label should be given using --pos_class argument"
            )
