...
def _calculate_and_output(
    ...
    if not os.path.exists(output_folder):
        ...
        logging.warning(
            "The output directory {} already exists. The data will be overwritten".format(
                output_folder
            )
        )

    if pickles:
        fragmentor_name = os.path.join(
            output_folder, ".".join([prop_name, calculator.short_name, "pkl"])
        )
        with open(fragmentor_name, "wb") as f:
            pickle.dump(calculator, f, pickle.HIGHEST_PROTOCOL)

    output_name = os.path.join(
        output_folder, ".".join([prop_name, calculator.short_name, fmt])
    )
    if fmt == "csv":
        ...
        dump_svmlight_file(
            np.array(desc, dtype="float32"), prop, output_name, zero_based=False
        )
...
def _perform_fullconfig(fullconfig: Dict[str, Any]) -> None:
    ...
    if (
        "numerical" in fullconfig.keys()
        or "solvent" in fullconfig.keys()
        or len(fullconfig["structures"].keys()) > 1
    ):
        data_x = data
    ...
    pool = mp.Pool(
        processes=fullconfig["parallel"] if fullconfig["parallel"] > 0 else 1
    )
    ...
    if args.morganfeatures:
        for nb in _set_default(args.morganfeatures_nBits, [1024]):
            for mr in _set_default(args.morganfeatures_radius, [2]):
                param_dict[_make_name(("morganfeatures", nb, mr))] = {
                    "nBits": nb,
                    "radius": mr,
                }
    if args.rdkfp:
    ...
    if args.rdkfplinear:
        for nb in _set_default(args.rdkfplinear_nBits, [1024]):
            for rl in _set_default(args.rdkfplinear_length, [3]):
                param_dict[_make_name(("rdkfplinear", nb, rl))] = {
                    "nBits": nb,
                    "radius": rl,
                }
    if args.layered:
        for nb in _set_default(args.layered_nBits, [1024]):
            for rl in _set_default(args.layered_length, [3]):
                param_dict[_make_name(("layered", nb, rl))] = {
                    "nBits": nb,
                    "radius": rl,
                }
    if args.avalon:
        ...
        for lower in _set_default(args.circus_min, [1]):
            for upper in _set_default(args.circus_max, [2]):
                if int(lower) <= int(upper):
                    if args.onbond:
                        param_dict[_make_name(("circus_b", lower, upper))] = {
                            "lower": lower,
                            "upper": upper,
                            "on_bond": True,
                        }
                    else:
                        param_dict[_make_name(("circus", lower, upper))] = {
                            "lower": lower,
                            "upper": upper,
                        }
    if args.linear:
        for lower in _set_default(args.linear_min, [2]):
            for upper in _set_default(args.linear_max, [5]):
                if int(lower) <= int(upper):
                    param_dict[_make_name(("chyline", lower, upper))] = {
                        "lower": lower,
                        "upper": upper,
                    }
    # if args.mordred2d:
...
def launch_preparer() -> None:
    ...
    # I/O arguments
    parser.add_argument(
        "-i", "--input", help="Input file, requires csv or Excel format"
    )
    parser.add_argument(
        "--structure_col",
        action="store",
    ...
    parser.add_argument(
        "-o", "--output", help="Output folder where the descriptor files will be saved."
    )
    parser.add_argument(
        "-f",
        "--format",
    ...
    # Morgan fingerprints
    parser.add_argument(
        "--morgan", action="store_true", help="Option to calculate Morgan fingerprints."
    )
    parser.add_argument(
        "--morgan_nBits",
        nargs="+",
    ...
    # RDKit fingerprints
    parser.add_argument(
        "--rdkfp", action="store_true", help="Option to calculate RDkit fingerprints."
    )
    parser.add_argument(
        "--rdkfp_nBits",
        nargs="+",
    ...
    # Avalon fingerprints
    parser.add_argument(
        "--avalon", action="store_true", help="Option to calculate Avalon fingerprints."
    )
    parser.add_argument(
        "--avalon_nBits",
        nargs="+",
    ...
    parser.add_argument(
        "--linear", action="store_true", help="Option to calculate ChyLine fragments."
    )
    parser.add_argument(
        "--linear_min",
        nargs="+",
    ...
    parser.add_argument(
        "--circus", action="store_true", help="Option to calculate CircuS fragments."
    )
    parser.add_argument(
        "--circus_min",
        nargs="+",
    ...
    if args.full_config:
        ...
        non_mordred_descriptors = [
            desc for desc in descriptor_dictionary.keys() if "mordred2d" not in desc
        ]
        # Use pool.map to apply the calculate_and_output function to each set of arguments in parallel
        # The arguments are tuples containing (inpt, descriptor, descriptor_params, output_params)
        pool.map(
            calculate_and_output,
            [
                (inpt, desc, descriptor_dictionary[desc], output_params)
                for desc in non_mordred_descriptors
            ],
        )
        pool.close()  # Close the pool and prevent any more tasks from being submitted
        pool.join()  # Wait for all the tasks to complete
